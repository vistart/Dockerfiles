...
基于 `postgres:10` 官方镜像构建，包含多个常用扩展。

## 已安装的扩展

- **PostGIS** 2.5 - 地理空间数据支持
- **Repack** - 在线重建索引和表
- **pg_cron** - 定时任务调度
...
# 启用扩展
docker exec postgres10 psql -U root -d test_db -c "CREATE EXTENSION IF NOT EXISTS postgis;"
docker exec postgres10 psql -U root -d test_db -c "CREATE EXTENSION IF NOT EXISTS pgvector;"
```
