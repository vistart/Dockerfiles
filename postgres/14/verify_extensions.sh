...
# 用法: DOCKER_HOST=tcp://host:2376 CONTAINER=postgres14 ./verify_extensions.sh
...
verify_extension() {
    ...
    echo "- $ext_name..."
    if $DOCKER_CMD exec "$CONTAINER" test -f "/usr/share/postgresql/14/extension/${ext_name}.control" 2>/dev/null; then
        echo "  ✓ $ext_name 已安装"
